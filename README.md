...
- **`/ask 你的结论`**：把这段话**用代码拆成一句一句**（句末标点、逗号、换行都算断句；`3.5` 这种小数点不算；太短的碎片如「嗯」「对吧」会并到相邻那句上），每句单独交给 Jev 判断，回复成一张核对清单：
...
- 一条 `/ask` 最多拆 **6 句**（多的部分并成最后一句，**不会丢内容**）；每一句都是一次评判调用，串行执行，但**只占 1 次提问额度**——长结论不该被罚。
...
| 一条 `/ask` 拆成好几句 | 逐句评判是**串行**的（不会为一条结论同时打出一堆请求），但仍然只占 **1 次**提问额度；每句都会占用全局并发名额 |
